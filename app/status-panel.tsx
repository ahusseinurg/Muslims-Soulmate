...
import { Camera, Clock, Eye, Plus, Sparkles } from 'lucide-react';
...
export default function StatusPanel({ statuses, user, now, name, onCreate, onView }: any) {
  ...
  const card = (s: any, index: number) => <button className={`status-card status-tone-${index % 5}`} key={s.id} onClick={() => onView(s)}>
    ...
    <span className="status-copy"><strong>{s.owner === user ? 'My status' : name(s.owner)}</strong><span>{s.body || (s.media ? 'Shared a photo or video' : 'New status')}</span><small><Clock size={13}/>{Math.max(1, Math.ceil((s.expires - now) / 3600000))}h left{s.owner === user && <><Eye size={13}/>{s.viewers?.length || 0}</>}</small></span>
  </button>;
  return <div className="status-page">
    ...
    <div className="safety-inline">You control who can view each status. Blocked members are always excluded. Do not share your home address, phone number, financial information, or live location.</div>
  </div>;
}
